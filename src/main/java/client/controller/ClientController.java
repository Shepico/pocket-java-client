...
//            showAlert("Неполные данные для авторизации!", Alert.AlertType.ERROR);
    ...
    public void receiveMessage(String message) {
        ...
        chatViewController.updateLastMessage(message, mfs.getSenderid(), false);//обновляем последнее сообщение
    }
    ...
    public void sendMessage(String message) {
        ...
        try {
            ...
            chatViewController.updateLastMessage(message, receiver.getUid(), true);//обновляем последнее сообщение
    ...
    private void synchronizeContactList() {
        ...
        for (User user1 : dbService.getAllUsers()) {
            if (user1.getUid() != myUser.getUid()) {
                ...
                //DataBaseService dbService = new DataBaseService(clientController.getMyUser());
